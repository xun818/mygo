...
export default function TeacherDashboard() {
  ...
  const deleteQuizWithQuestions = async quizId => {
    ...
    try {
      const rsp = await fetch(`http://localhost:8000/quiz/${quizId}`, { method: "DELETE" });
  ...
  return (
    <div className="teacher-dashboard">
      ...
      <section className="section verify-section" style={{ marginTop: 40 }}>
        ...
        <ul className="list" style={{ marginTop: 12, maxHeight: 200, overflowY: "auto" }}>
          {examList.length === 0 ? (
            <li>此學生尚未上傳答案卷</li>
          ) : (
            examList.map(exam => (
        ...
        {verifyStatus && (
          <pre style={{ marginTop: 8, whiteSpace: "pre-wrap" }}>{verifyStatus}</pre>
        )}
      </section>
    </div>
  );
